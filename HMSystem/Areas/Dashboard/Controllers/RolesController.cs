...
namespace HMSystem.Areas.Dashboard.Controllers
{
    public class RolesController : Controller
    {
        ...
        {
            var role = RoleManager.Roles.AsQueryable();

            if (!string.IsNullOrEmpty(searchTerm))
            {
                role = role.Where(a => a.Name.ToLower().Contains(searchTerm.ToLower()));
            }

            var skip = (page - 1) * recordSize;

            return role.OrderBy(x => x.Name).Skip(skip).Take(recordSize).ToList();
        }
